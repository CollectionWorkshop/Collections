    ...
    public class Linq101
    {
        ...
        private static void SimpleDataGrouping(IEnumerable<GameData> footballData)
        {
            var groupedWinsByYear = footballData
                ...
                .Select(f => new Tuple<int, int>(f.Key, f.Count())) // Wybierzmy dane w parze rok - ilosc wygranych
                .ToDictionary(k => k.Item1, v => v.Item2) // wpiszmy dane tymczasowo do słownika żeby je jeszcze posortować
        ...
        private static void QueryGamesAwayVsHomeAnalysis(IEnumerable<GameData> footballData)
        {
            ...
            var homeWinningsperCountry = footballData
                ...
                .Select(f => new Tuple<string, int>(f.Key, f.Count())) // wybierzmy liczebność każdej z grup
                .ToDictionary(k => k.Item1, v => v.Item2); // wpiszmy dane do słownika kraj-ilość wygranych
            ...
            var totalGamesPerCountry = footballData
                ...
                .Select(f => new Tuple<string, int>(f.Key, f.Count())) // zbierzmy liczebność każdej z grup
                .ToDictionary(k => k.Item1, v => v.Item2); // wpiszmy dane do słownika kraj-ilość meczów
